...
from server import _create_env, _delete_env, _read_file
...
def test_binary_and_image():
    ...
    try:
        ...
        with open(f"/Users/cobaltcore/.mcp-python-executor/envs/{env_id}/test.png", "wb") as f:
            f.write(png_data)
        ...
        with open(f"/Users/cobaltcore/.mcp-python-executor/envs/{env_id}/test.bin", "wb") as f:
            f.write(b"\x00\x01\x02\x03")

        res = _read_file(env_id, "test.bin")
        if isinstance(res, str) and "Binary file" in res:
            print("Success: Detected as Binary string")
        else:
